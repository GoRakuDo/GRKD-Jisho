...
- [ ] **1-10** Rate Limit サービス実装
  ...
  - Owner / Administrator は無制限（DB 参照省略）
   - 上限超過時は通常のチャンネル返信（`message.reply()`）
...
- [ ] **2-4** `/refresh-jisho <word> [role]` 実装
  - 指定クエリ（+ ロール）のキャッシュを削除。ただし `is_delete_protected = true` は削除しない
  - 再生成は次の検索時に自動で行う
...
- [ ] **R-5** Security release gate
  ...
  - wipe safety と delete protection が崩れていないことを確認する
...
- [ ] **5-4** MCP Level 4 dangerous tools 実装
  ...
  - `grkd-jisho.request_bulk_cache_delete`（一括cache削除、delete protection 除外）
