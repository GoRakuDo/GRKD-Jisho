...
> - 毎日 00:00 GMT+7 に wipe_enabled チャンネルがクローン方式で自動消去
...
export async function wipeChannel(discordChannel: TextChannel): Promise<{
  newChannelId: string;
  deletedCount: number;
}> {
  // Step 1: 固定メッセージを退避
  ...
  // Step 5: Bot のキャッシュが更新されるのを待つ
  await new Promise((resolve) => setTimeout(resolve, 1000));

  return {
    newChannelId: newChannel.id,
    deletedCount: pinContents.length,
  };
}
```
...
- [ ] **1-11:** クローン方式でチャンネルが空になる。固定メッセージが保持される。`channel_id` が新IDに更新される
...
| 5 | クローン後に `channel_settings.channel_id` を更新し忘れる | Wipe が動かなくなる | cron 内で必ず DB 更新するコードにしている |
| 6 | `bot_events` への INSERT が遅い | 返答が遅延する | 非同期 fire-and-forget。`await` しない or 別キューに分離 |
| 7 | `node-cron` のタイムゾーン依存 | 夏時間でズレる？ | Asia/Bangkok は夏時間なしなので安全 |
