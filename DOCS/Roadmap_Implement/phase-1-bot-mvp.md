...
- language guard fail 時は同じ provider で最大2回 ReAsk し、それでも fail なら fallback model に回す。fallback 出力も fail した場合は cache 保存も Discord 返信もしない。
...
- `packages/bot/src/services/language-guard.service.ts` にスクリプト検出 + 同一文字ゴミ + English ratio validator を実装し、`llm.service.ts` の `generateWithLanguageGuardrails()` で Gemini ReAsk → OpenRouter fallback の順に制御するようにした。
