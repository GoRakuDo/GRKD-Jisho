...
- cache は `prompt.version` に加えて `prompt.content` hash も含めていた（当時 7要素）。同じ version を編集保存しても古い cache を踏まない設計だったが、prompt 編集ごとに過去 cache が全て無効化される問題があった。
- **2026-06-21 変更**: `prompt.content` hash を cache key から除外（6要素）。hash カラムは DB に残し編集履歴・analytics メタ用に。prompt 編集時は必ず `prompt_version` を bump して cache を無効化する運用ルール。詳細は `DOCS/Design/cache-key-prompt-version-only.md`。
