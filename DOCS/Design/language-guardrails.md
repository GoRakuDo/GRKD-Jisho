...
LLM に渡す直前の `definition_json` で以下の置換を行う：
- `content: "━"` / `content: "―"` の完全一致ノード → `dictionary_form`
- `「…━…」` / `「…―…」` を含む文字列 → `「」` 内の `━` / `―` を全て `dictionary_form` に置換
- 該当 placeholder が無い辞書は no-op

`・` 分割や `examples_normalized` は第2段階候補として扱う。
...
OpenRouter の transport timeout / JSON parse failure は既存どおり 150 秒 × 最大2回。  
