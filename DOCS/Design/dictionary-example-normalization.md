...
> **Status:** Phase 1.5 implemented (2026-06-15)
> **Phase:** Phase 4 quality optimization — search/input quality follow-up
> **Implementation status:**
> - Phase 1 (commit cc4830d): `content: "━"` / `content: "―"` exact-match replacement
> - Phase 1.5 (current): `「…━…」` / `「…―…」` string-level replacement
> - Phase 2 (planned): `・` example splitting, `〔＝…〕` note extraction
>
...
| 位置 | 例 | 正規化 | Phase |
|---|---|---|---|
| `content` 値が完全一致 `"━"` | `{"content":"━"}` | `{"content":"軟派"}` | Phase 1 |
| `content` 値が完全一致 `"―"` | `{"content":"―"}` | `{"content":"軟派"}` | Phase 1 |
| `「」` 内に `━` / `―` を含む文字列 | `"「━の不良少年」"` | `"「軟派の不良少年」"` | Phase 1.5 |
| `「」` 内のプレースホルダ (Kasou 広辞苑型) | `"「―議員」"` | `"「軟派議員」"` | Phase 1.5 |
| `「」` 外の `━` / `―` | `"━の不良少年"` | no-op | Phase 2 候補 |
...
### Step 1.5 — 「」内の placeholder 置換（2026-06-15 追加）

Step 1 だけだと広辞苑の「―議員」型が `dictionaryForm` に置換されず LLM に raw 記号が残る。

ルール：
- `content` 文字列内の `「…━…」` / `「…―…」` が出現した時だけ、`「」` 内の `━` / `―` を全て `dictionaryForm` に置換
- `「」` 外の `━` / `―` は触らない（Phase 2 で `・` 分割と一緒に扱う予定）
- 正規表現: `「([^」]*[━―][^」]*)」` → 内側の `replace(/[━―]/g, dictionaryForm)`
...
Phase 1.5 テスト:
- 「」内の ━ → dictionaryForm に置換
- 「」内の ― → dictionaryForm に置換
- 「」外の ━ / ― → no-op
- 複数の「」をまとめて置換
- Kasou 実データ（広辞苑 存ずる）で「」内 ― が置換される
...
| リスク | 対策 |
|---|---|
| `・` を品詞表記まで分割する | Phase 2 では `・` を分割しない（`「」` 内の `━`/`―` だけ置換） |
| `query` が活用形のまま代入される | `dictionaryForm` を使う |
| 既に完全な例文まで変換してしまう | `content` 完全一致 + `「」` 内の placeholder だけ置換。`「」` 外の `━` / `―` は no-op |
| `〔＝...〕` 補足を独立用例として扱う | Phase 2 では構造化しない。`note` に紐づけ、直前用例から切り離さない |
| LLM が `・` 区切りをまだ合成する | まず置換だけで様子を見る。残る場合だけ第2段階で `examples_normalized` を入れる |
| ネストした `「」` の中の `━` / `―` | `「」` パターマッチが内側の `」` で切れるため外側の `━` は対象外。Yomitan 辞書では実例なし |

---

## 完了条件

- `content: "━"` / `content: "―"` の完全一致ノードが `dictionaryForm` に置換されて LLM に渡る。（Phase 1）
- `「」` 内の `━` / `―` が `dictionaryForm` に置換され、`「」` 外は no-op。（Phase 1.5）
- 実 Kasou データ（広辞苑 存ずる / 軟派）での spot-check 通過。（Phase 1.5）
- DB の生データは変更されない。
- `dictionaryForm` が空の場合は no-op。
