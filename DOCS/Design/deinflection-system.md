...
- `@bot 止事無き` → `query = "止事無き"` → deinflect → `"止事無い"` で辞書ヒット
...
| 〜ている | ている → て | 思っている → 思って |
| 文語形容詞（限定） | 無き → 無い / なき → ない | 止事無き → 止事無い |
...
Yomitan 辞書には、現代語見出し `止事無い` / `やんごとない` が入っていても、ユーザー入力では文語的な連体形 `止事無き` / `やんごとなき` が来ることがある。

このケースを放置すると、greedy scan は長い語を見つけられず、最終的に短い漢字1文字（例: `止`）で辞書ヒットしてしまう。

```txt
入力: 止事無き
現行NG: 止事無き → no hit → 止事無 → no hit → ... → 止 ✅（誤ヒット）
期待OK: 止事無き → deinflect → 止事無い ✅
```

ただし、文語変換を広げすぎると表記ゆれ推測になりやすい。
そのため Phase 4 時点では、実データで確認済みの次の最小ルールだけを入れる。
...
このルールは「辞書にある現代語見出しへ戻すための末尾置換」であり、意味補完ではない。
新しい文語ルールを追加する場合は、必ず実DBに戻り先の見出し語が存在すること、かつ短い語への誤ヒットを防ぐテストを先に追加する。
...
| `止事無き` | `"止事無き"`（lookup結果は `止事無い` / `止ん事無い` 側へ戻る） |
| `cat` | `null`（英語のみなので非対応） |
