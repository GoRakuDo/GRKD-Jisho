...
| **L3 Limited write** | L1 + `request_*` 系（ops_jobs作成） | `MCP_READONLY_MODE=false` + `MCP_ENABLE_DRY_RUN=true` + `MCP_ENABLE_LIMITED_WRITE=true` |
...
(7) grkd-jisho.wipe_status         ← wipe 失敗レコードがないか
      ↓ wipe 失敗あり + L2以上 → grkd-jisho.wipe_status
```
...
### 2-1. grkd-jisho.dry_run_rate_limit_change
...
### 2-2. grkd-jisho.dry_run_cache_refresh
...
| wipe 失敗 | Bot権限不足 / チャンネル削除 | wipe_status |
