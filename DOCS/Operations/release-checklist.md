...
|---|---|---|---|
| DB typecheck | `pnpm --filter @grkd-jisho/db exec tsc --noEmit` | not tested | R-1作成時点では未実行 |
| Bot typecheck | `pnpm --filter @grkd-jisho/bot exec tsc --noEmit` | not tested | R-1作成時点では未実行 |
| MCP typecheck | `pnpm --filter @grkd-jisho/mcp exec tsc --noEmit` | not tested | R-1作成時点では未実行 |
| Web typecheck | `pnpm --filter @grkd-jisho/web typecheck` | not tested | R-1作成時点では未実行 |
| Web build | `pnpm --filter @grkd-jisho/web build` | not tested | R-1作成時点では未実行 |
| Bot tests | `pnpm --filter @grkd-jisho/bot test` | not tested | R-1作成時点では未実行 |
...
| Pattern scan | `as any`, `eslint-disable`, `Asia/Bangkok`, `@grkd/`, `grkd.`, pure black/white がコードに残らない | not tested | `Asia/Bangkok` は旧timezone残骸検出用 |
| code-reviewer | BLOCKER/HIGH 0件 | not tested |  |
...
| Item | Status |
|---|---|
| 自動検証 pass | not tested |
| 手動検証 fail 0件 | not tested |
