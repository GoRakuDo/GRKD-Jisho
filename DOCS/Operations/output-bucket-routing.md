...
- `indonesian` uses the same allowed script set, plus English stopword ratio ≤ 10%. If validation fails, the bot should ReAsk the current model twice; if it still fails, it falls back to the next `models.json` priority entry, which also gets the same two ReAsk attempts.
