...
const { mockUpdateResponse, mockDeleteCacheByQuery, mockGetResponseById } = vi.hoisted(() => {
  return {
    ...
    mockGetResponseById: vi.fn<(id: string) => Promise<{ id: string; query: string; roleKey: string; modelName: string; modelDisplayName: string; modelProvider: string; promptVersion: string; isManualOverride: boolean; isDeleteProtected: boolean; updatedAt: Date | null; responseText: string } | null>>(),
  };
});
...
const makeResult = (overrides: Partial<SearchResult> = {}): SearchResult => ({
  ...
  modelDisplayName: "OpenRouter Gemma 4 31B IT",
  modelProvider: "CPA",
  promptVersion: "v1",
