...
### 15-1. code-reviewer 機械ゲート（撤去済み・通常運用へ移行）

かつて `pre-push` hook や `.review/approved/` マーカーによる機械チェックを運用していたが、ワークフロー整理に伴い hook およびチェックスクリプトは撤去された。
コードレビューは必要に応じてエージェント主導で実施し、通常の `git push` で安全にデプロイする運用とする。
